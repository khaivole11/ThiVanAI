...
const poetryForms = Object.entries(POETRY_FORM_LABELS) as [PoetryForm, string][]
...
const processSteps = [
  { step: '01', title: 'Nhập câu thơ', desc: 'Bạn viết một câu thơ mở đầu và chọn thể thơ.' },
  { step: '02', title: 'Truy xuất thi ca', desc: 'Hệ thống tìm các bài thơ liên quan trong kho dữ liệu tiếng Việt.' },
  { step: '03', title: 'Lựa chọn ngữ cảnh', desc: 'Những tác phẩm phù hợp nhất được chọn làm tài liệu tham khảo.' },
  { step: '04', title: 'Sáng tác bài thơ', desc: 'Mô hình tạo bài thơ mới kết hợp yêu cầu và ngữ cảnh đã chọn.' },
]
...
export default function LandingPage() {
  ...
  function handleStart() {
    ...
    navigate('/sang-tac', { state: { openingVerse: openingVerse.trim(), poetryForm: selectedForm } })
  }
  ...
  return (
    <div>
      {/* Hero */}
      <section className="bg-[#fcf8f1] border-b border-[#e4e1da]" aria-labelledby="hero-heading">
        <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-12 pt-16 pb-20 md:pt-24 md:pb-28">
          <div className="max-w-3xl mx-auto text-center">
            ...
            >
              Biến một câu thơ<br />
              <span className="text-[#3f4a6b]">thành một bài thơ hoàn chỉnh.</span>
            </h1>

            <p className="text-lg md:text-xl text-[#5f6673] leading-relaxed max-w-2xl mx-auto mb-10">
              Nhập câu thơ mở đầu, chọn thể thơ và phong cách. Hệ thống sẽ tìm những tác phẩm liên quan để hỗ trợ sáng tác một bài thơ tiếng Việt mới.
            </p>

            {/* Quick start */}
            <div className="bg-white rounded-2xl border border-[#e4e1da] shadow-sm p-6 max-w-2xl mx-auto text-left">
              <div className="mb-4">
                <label htmlFor="opening-verse" className="block text-sm font-semibold text-[#252932] mb-2">
                  Câu thơ mở đầu
                </label>
                <textarea
                  id="opening-verse"
                  ...
                  onChange={(e) => {
                    ...
                    if (errors.verse) setErrors((prev) => ({ ...prev, verse: undefined }))
                  }}
                  ...
                  className={`w-full px-4 py-3 rounded-lg border text-base text-[#252932] placeholder:text-[#a8adb5] resize-none focus:outline-none focus-visible:ring-2 focus-visible:ring-[#596789] transition-colors ${
                    errors.verse ? 'border-[#b54747] bg-[#fceeee]' : 'border-[#d5d2ca] focus:border-[#7c89aa]'
                  }`}
                  style={{ fontFamily: "'Be Vietnam Pro', sans-serif" }}
                />
                {errors.verse && (
                  <p id="verse-error" role="alert" className="mt-1.5 text-sm text-[#b54747]">
                    {errors.verse}
                  </p>
                )}

                {/* Example prompts */}
                <div className="flex flex-wrap gap-2 mt-2" role="group" aria-label="Gợi ý câu thơ">
              ...
              <div className="mb-5">
                <label htmlFor="poetry-form" className="block text-sm font-semibold text-[#252932] mb-2">
                  Thể thơ
                </label>
                <select
                  id="poetry-form"
                ...
                {errors.form && (
                  <p id="form-error" role="alert" className="mt-1.5 text-sm text-[#b54747]">
                    {errors.form}
                  </p>
                )}
              </div>
      ...
      <section className="py-16 md:py-20" aria-labelledby="benefits-heading">
        <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-12">
          <h2 id="benefits-heading" className="text-2xl md:text-3xl font-bold text-[#252932] text-center mb-3">
            Tại sao dùng Thi Vận AI?
          </h2>
          <p className="text-[#5f6673] text-center mb-12 max-w-xl mx-auto">
            Không chỉ là trợ lý AI thông thường — hệ thống kết hợp kho thi ca tiếng Việt với công nghệ truy xuất thông minh.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {benefits.map((benefit) => (
              <article key={benefit.title} className="bg-white rounded-xl border border-[#e4e1da] p-6 hover:border-[#d6b98c] hover:shadow-sm transition-all">
                <div className="w-10 h-10 rounded-lg bg-[#f2f4f8] text-[#3f4a6b] flex items-center justify-center mb-4">
                  {benefit.icon}
                </div>
                <h3 className="font-semibold text-[#252932] text-lg mb-2">{benefit.title}</h3>
                <p className="text-[#5f6673] text-sm leading-relaxed">{benefit.text}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 md:py-20 bg-[#f4f2ed]" aria-labelledby="process-heading">
        <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-12">
          <div className="text-center mb-12">
            <h2 id="process-heading" className="text-2xl md:text-3xl font-bold text-[#252932] mb-3">
              Hệ thống hoạt động như thế nào?
            </h2>
            <p className="text-[#5f6673] max-w-xl mx-auto">
              Ứng dụng không gửi yêu cầu trực tiếp đến mô hình ngôn ngữ. Trước tiên, hệ thống tìm bài thơ liên quan và dùng chúng làm ngữ cảnh.
            </p>
          </div>

          <div className="relative">
            {/* Connector line (desktop) */}
            <div className="hidden lg:block absolute top-8 left-[12.5%] right-[12.5%] h-0.5 bg-[#d5d2ca]" aria-hidden="true" />
              ...
              {processSteps.map((step) => (
                <li key={step.step} className="relative flex flex-col items-center text-center">
                  <div className="w-16 h-16 rounded-full bg-white border-2 border-[#d6b98c] flex items-center justify-center mb-4 z-10">
                    <span className="font-bold text-[#3f4a6b] text-lg font-mono">{step.step}</span>
                  </div>
                  <h3 className="font-semibold text-[#252932] mb-2">{step.title}</h3>
                  <p className="text-sm text-[#5f6673] leading-relaxed">{step.desc}</p>
                </li>
              ))}
            </ol>
          </div>
          ...
          <div className="text-center mt-10">
            <Button variant="secondary" onClick={() => navigate('/cach-hoat-dong')}>
              Tìm hiểu chi tiết →
            </Button>
          </div>
        </div>
      ...
      <section className="py-16 md:py-20" aria-labelledby="example-heading">
        <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 id="example-heading" className="text-2xl md:text-3xl font-bold text-[#252932] mb-4">
                Bài thơ được tạo ra trông như thế nào?
              </h2>
              <p className="text-[#5f6673] leading-relaxed mb-6">
                Từ một câu thơ mở đầu đơn giản, hệ thống phát triển thành bài thơ hoàn chỉnh, tuân theo thể thơ đã chọn và sử dụng các tác phẩm tiếng Việt làm tham chiếu.
              </p>
              <div className="bg-[#f4f2ed] rounded-lg p-4 text-sm">
                <p className="text-[#7d8490] font-medium mb-2">Câu thơ mở đầu:</p>
                <p className="text-[#252932] italic font-serif">"Trăng nghiêng qua mái hiên nhà"</p>
              </div>
            </div>
      ...
      {/* Poetry forms */}
      <section className="py-16 md:py-20 bg-[#fcf8f1] border-t border-[#e4e1da]" aria-labelledby="forms-heading">
        <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-12">
          <h2 id="forms-heading" className="text-2xl font-bold text-[#252932] text-center mb-8">
            Thể thơ được hỗ trợ
          </h2>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-12">
          <div className="bg-[#f2f4f8] rounded-2xl p-8 md:p-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              <div>
                <h2 id="dataset-heading" className="text-2xl font-bold text-[#252932] mb-3">
                  Kho dữ liệu thi ca tiếng Việt
                </h2>
                <p className="text-[#5f6673] leading-relaxed mb-5">
                  Hệ thống dựa trên kho thơ tiếng Việt được thu thập từ nhiều nguồn uy tín, bao gồm thơ cổ điển và hiện đại.
                </p>
                <Button variant="secondary" size="sm" onClick={() => navigate('/ve-du-an')}>
                  Xem thông tin dự án →
                </Button>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={item.source} className="bg-white rounded-xl border border-[#e4e1da] p-4 text-center">
                    <div className="text-xl font-bold text-[#3f4a6b]">{item.count}</div>
                    <div className="text-xs text-[#7d8490] mt-0.5">{item.label}</div>
                    <div className="text-xs font-medium text-[#5f6673] mt-1">{item.source}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* CTA */}
      <section className="py-16 md:py-20 bg-[#3f4a6b]" aria-labelledby="cta-heading">
        <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-12 text-center">
          ...
          <p className="text-[#a5aec7] text-lg mb-8 max-w-lg mx-auto">
            Nhập một câu thơ của bạn và để hệ thống phát triển thành tác phẩm hoàn chỉnh.
          </p>
          <Button
            size="lg"
