...
export default defineConfig(({ command, mode }) => {
  ...
  const frontendHost = env.FRONTEND_DEV_HOST?.trim() || "0.0.0.0";
  const frontendPort = Number.parseInt(
    env.FRONTEND_DEV_PORT?.trim() || "8443",
    10,
  );
  const backendProxyTarget = env.BACKEND_PROXY_TARGET?.trim();
  const figmaPublicUrl = env.FIGMA_PUBLIC_URL?.trim();
  ...
  if (
    ...
    throw new Error("FRONTEND_DEV_PORT must be an integer between 1 and 65535");
  }

  if (command === "serve" && mode === "development" && !backendProxyTarget) {
    throw new Error("BACKEND_PROXY_TARGET is required in development mode");
  }

  const apiProxy = backendProxyTarget
    ? {
        "/api": {
  ...
  return {
    base: figmaPublicUrl ? `${figmaPublicUrl.replace(/\/+$/, "")}/` : "/",
