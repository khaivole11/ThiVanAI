    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#e4e1da] flex-shrink-0">
          <div>
            <p className="text-xs text-[#7d8490] mb-0.5">
              {currentIndex !== undefined && total !== undefined ? `Nguồn ${currentIndex + 1} / ${total}` : 'Chi tiết bài thơ'}
            </p>
            <h2 className="font-semibold text-[#252932] text-base truncate max-w-xs">{source.title}</h2>
          </div>
          <button
            onClick={onClose}
            aria-label="Đóng"
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide p-5 space-y-5">
          {/* Metadata */}
          <section>
            <h3 className="text-xs font-semibold uppercase tracking-wide text-[#7d8490] mb-3">Thông tin bài thơ</h3>
            <dl className="space-y-2">
              <div className="flex gap-2">
                <dt className="text-sm text-[#7d8490] w-24 flex-shrink-0">Tác giả</dt>
                <dd className="text-sm text-[#252932] font-medium">{source.author || 'Chưa rõ tác giả'}</dd>
              </div>
              <div className="flex gap-2">
                <dt className="text-sm text-[#7d8490] w-24 flex-shrink-0">Thể thơ</dt>
                <dd><Badge variant="outline">{source.poetryForm}</Badge></dd>
              </div>
              <div className="flex gap-2">
                <dt className="text-sm text-[#7d8490] w-24 flex-shrink-0">Thời kỳ</dt>
                <dd className="text-sm text-[#252932]">{source.period || 'Chưa xác định'}</dd>
              </div>
              <div className="flex gap-2">
                <dt className="text-sm text-[#7d8490] w-24 flex-shrink-0">Hạng truy xuất</dt>
                <dd className="text-sm text-[#252932] font-medium">#{source.rank}</dd>
              </div>
            </dl>
          </section>
          ...
          {/* Why selected */}
          <section>
            <h3 className="text-xs font-semibold uppercase tracking-wide text-[#7d8490] mb-3">Lý do được chọn</h3>
            ...
            <p className="text-sm text-[#5f6673] leading-relaxed">
              Bài thơ này được chọn vì {source.matchTags.map((t) => t.label.toLowerCase()).join(', ')} với yêu cầu của bạn.
            </p>
          </section>

          {/* Excerpt */}
          <section>
            <h3 className="text-xs font-semibold uppercase tracking-wide text-[#7d8490] mb-3">Trích đoạn</h3>
            <blockquote className="bg-[#fffcf7] border border-[#e4e1da] rounded-lg p-4">
              <p className="font-serif text-[#292823] leading-[1.9] whitespace-pre-line text-base">{source.excerpt}</p>
            </blockquote>
          </section>
          ...
          {showScores && (
            <section>
              <h3 className="text-xs font-semibold uppercase tracking-wide text-[#7d8490] mb-3">Điểm truy xuất (Nghiên cứu)</h3>
              <div className="bg-[#f4f2ed] rounded-lg p-3 grid grid-cols-2 gap-3 font-mono text-sm">
                <div>
                  ...
                  <div className="text-[#3f4a6b] font-medium">{source.denseScore?.toFixed(4)}</div>
                </div>
                <div>
                  <span className="text-[#7d8490] text-xs">BM25 score</span>
                  <div className="text-[#3f4a6b] font-medium">{source.bm25Score?.toFixed(4)}</div>
                </div>
                <div>
                  <span className="text-[#7d8490] text-xs">Hybrid score</span>
                  <div className="text-[#3f4a6b] font-medium">{source.hybridScore?.toFixed(4)}</div>
                </div>
                <div>
          ...
          <div className="bg-[#edf5fa] border border-[#b8d5e7] rounded-lg p-3 text-xs text-[#2c5271] leading-relaxed">
            Tác phẩm này được sử dụng làm ngữ cảnh tham khảo cho hệ thống. Bài thơ được tạo không sao chép nguyên văn nội dung nguồn.
          </div>
        </div>
