...
type Step = 'idle' | 'analyzing' | 'retrieving' | 'selecting' | 'generating' | 'complete' | 'error'
...
export default function PoetryGenerator() {
  ...
  const [openingVerse, setOpeningVerse] = useState(locationState?.openingVerse ?? '')
  const [poetryForm, setPoetryForm] = useState<PoetryForm | ''>(locationState?.poetryForm ?? '')
  const [authorStyle, setAuthorStyle] = useState(locationState?.authorStyle ?? '')
  const [period, setPeriod] = useState<LiteraryPeriod>(locationState?.period ?? '')
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setStep('error')
      setGenerationError('Không thể gửi yêu cầu tạo thơ. Vui lòng kiểm tra kết nối và thử lại.')
    }
  }
  ...
  if (isGenerating) {
    return <GenerationProgress currentStepIdx={currentStepIdx} openingVerse={openingVerse} poetryForm={poetryForm as PoetryForm} onCancel={() => { setStep('idle'); setCurrentStepIdx(-1) }} />
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-12 py-10">
      ...
      <header className="mb-8">
        <h1 className="text-3xl font-bold text-[#252932] mb-2">Sáng tác bài thơ mới</h1>
        <p className="text-[#5f6673]">Bắt đầu bằng một câu thơ của bạn, sau đó chọn thể thơ và phong cách phù hợp.</p>
      </header>

      {generationError && (
        <div role="alert" className="mb-6 bg-[#fceeee] border border-[#edb8b8] text-[#8e3030] rounded-lg px-4 py-3 text-sm flex items-start gap-3">
          <span className="font-bold flex-shrink-0">✕</span>
          <div>
            <p className="font-medium">{generationError}</p>
            <button onClick={handleGenerate} className="mt-2 text-[#8e3030] underline hover:no-underline text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b54747] rounded">
              Thử lại
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-xl border border-[#e4e1da] p-6 space-y-6">
            ...
            <fieldset>
              <legend className="flex items-center gap-2 mb-2">
                <span className="text-sm font-semibold text-[#252932]">Câu thơ mở đầu</span>
              ...
              <div className="flex justify-between mt-1.5">
                <span id="verse-help" className="text-xs text-[#7d8490]">Câu thơ này sẽ được giữ làm điểm khởi đầu.</span>
                <span
                  id="verse-count"
              ...
              {errors.verse && (
                <p id="verse-error" role="alert" className="mt-1 text-sm text-[#b54747]">{errors.verse}</p>
              )}

              {/* Example chips */}
              <div className="mt-3">
                <p className="text-xs text-[#7d8490] mb-2">Thử câu thơ gợi ý:</p>
                <div className="flex flex-wrap gap-1.5" role="group" aria-label="Câu thơ gợi ý">
                  {EXAMPLE_PROMPTS.map((p) => (
                    <button
                      key={p}
                      onClick={() => { setOpeningVerse(p); setErrors((prev) => ({ ...prev, verse: undefined })) }}
            ...
            {/* Poetry form */}
            <div>
              <label htmlFor="poetry-form" className="flex items-center gap-2 mb-2">
                <span className="text-sm font-semibold text-[#252932]">Thể thơ</span>
                <Badge variant="error">Bắt buộc</Badge>
              </label>
              <select
                id="poetry-form"
              ...
              >
                ...
                {(Object.entries(POETRY_FORM_LABELS) as [PoetryForm, string][]).map(([v, l]) => (
              ...
              {errors.form && (
                <p id="form-error" role="alert" className="mt-1 text-sm text-[#b54747]">{errors.form}</p>
              )}
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-[#7d8490] font-medium">Tùy chọn</span>
                <div className="flex-1 h-px bg-[#e4e1da]" />
              </div>

              {/* Author */}
              <div className="relative">
                <label htmlFor="author-style" className="flex items-center gap-2 mb-2">
                  <span className="text-sm font-medium text-[#252932]">Phong cách tác giả</span>
                ...
                {showSuggestions && authorSuggestions.length > 0 && (
                  ...
                  >
                    {authorSuggestions.map((author) => (
                      <li key={author} role="option" aria-selected={authorStyle === author}>
                        <button
                          className="w-full text-left px-4 py-2.5 text-sm text-[#252932] hover:bg-[#f2f4f8] transition-colors"
                          onMouseDown={() => { setAuthorStyle(author); setShowSuggestions(false) }}
                        >
                          {author}
                        </button>
                      </li>
                ...
                <p id="author-help" className="mt-1.5 text-xs text-[#7d8490]">
                  Dùng làm tiêu chí tham khảo phong cách, không sao chép nguyên văn.
                </p>
              </div>

              {/* Period */}
              <div>
                <label htmlFor="period" className="block text-sm font-medium text-[#252932] mb-2">
                  Thời kỳ sáng tác
                </label>
                <select
                  id="period"
              ...
              <div className="bg-[#edf5fa] border border-[#b8d5e7] rounded-lg p-3 text-xs text-[#2c5271] leading-relaxed flex gap-2">
                ...
                <span>Hệ thống tìm bài thơ phù hợp với ít nhất một tiêu chí đã chọn (logic OR).</span>
              </div>
            </div>
            ...
            <div>
              ...
              {showAdvanced && (
                <div className="mt-4 pt-4 border-t border-[#e4e1da] space-y-4 animate-fade-in">
                  <div>
                    <label htmlFor="top-k" className="block text-sm font-medium text-[#252932] mb-2">
                      Số bài thơ tham khảo: <span className="text-[#3f4a6b] font-semibold">{topK}</span>
                    </label>
                    <input
                      id="top-k"
                      type="range"
                    ...
                    />
                    <div className="flex justify-between text-xs text-[#7d8490] mt-1" id="topk-help">
            ...
            <div className="flex flex-col gap-3 pt-2">
              ...
              <Button onClick={handleReset} variant="ghost" size="md" className="w-full">
                Đặt lại
              </Button>
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-[#fffcf7] border border-[#e4e1da] rounded-xl min-h-80 flex flex-col items-center justify-center p-12 text-center">
            <div aria-hidden="true" className="w-16 h-16 rounded-full bg-[#f4f2ed] flex items-center justify-center mb-5">
              <span className="text-2xl text-[#d6b98c] font-serif">詩</span>
            </div>
            <h2 className="text-xl font-semibold text-[#252932] mb-2" style={{ fontFamily: "'Lora', serif" }}>
          ...
          <div className="bg-white rounded-xl border border-[#e4e1da] p-6">
            <h3 className="font-semibold text-[#252932] mb-4">Hướng dẫn nhanh</h3>
            ...
            return (
              ...
              >
                <span className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                  done ? 'bg-[#4f7a68] text-white' : active ? 'bg-[#3f4a6b] text-white' : 'bg-[#e4e1da] text-[#7d8490]'
                }`}>
                  {done ? '✓' : i + 1}
                </span>
                <span className={`text-sm font-medium ${active ? 'text-[#3f4a6b]' : done ? 'text-[#5f6673]' : 'text-[#a8adb5]'}`}>
                  {step.label}
                  {active && '...'}
                </span>
              </li>
        ...
        <div className="bg-[#f4f2ed] rounded-lg p-4 mb-6 text-sm space-y-1">
          ...
          {poetryForm && (
            <div className="mt-2 flex items-center gap-2">
              ...
              <Badge variant="secondary">{POETRY_FORM_LABELS[poetryForm]}</Badge>
            </div>
          )}
        </div>
