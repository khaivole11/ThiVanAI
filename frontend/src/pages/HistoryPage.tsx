...
import { POETRY_FORM_LABELS, PERIOD_LABELS, type GeneratedPoem } from '../types'
...
export default function HistoryPage() {
  ...
  if (search.trim()) {
    ...
    filtered = filtered.filter(
      (p) => p.title.toLowerCase().includes(q) || p.openingVerse.toLowerCase().includes(q),
    )
  }
  if (formFilter) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-8 lg:px-12 py-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-[#252932]">Lịch sử sáng tác</h1>
          <p className="text-[#5f6673] mt-1">Xem lại, chỉnh sửa hoặc tạo phiên bản mới từ những bài thơ trước đây.</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        {poems.length > 0 && (
          <Button variant="ghost" size="sm" onClick={() => setClearConfirm(true)} className="text-[#b54747] hover:bg-[#fceeee]">
            Xóa tất cả
          </Button>
        )}
      </div>
      ...
      {poems.length === 0 ? (
        <div className="text-center py-24 bg-white rounded-xl border border-[#e4e1da]">
          <div className="w-16 h-16 rounded-full bg-[#f4f2ed] flex items-center justify-center mx-auto mb-5" aria-hidden="true">
            <span className="text-2xl font-serif text-[#d6b98c]">詩</span>
          </div>
          <h2 className="text-xl font-semibold text-[#252932] mb-2">Bạn chưa có bài thơ nào.</h2>
          <p className="text-[#5f6673] mb-6">Hãy bắt đầu bằng một câu thơ của riêng bạn.</p>
          <Button onClick={() => navigate('/sang-tac')}>Tạo bài thơ đầu tiên</Button>
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-xl border border-[#e4e1da]">
          <h2 className="text-lg font-semibold text-[#252932] mb-2">
            {search ? 'Không tìm thấy bài thơ phù hợp với từ khóa.' : 'Không có bài thơ nào phù hợp với bộ lọc hiện tại.'}
          </h2>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4" role="list" aria-label="Danh sách bài thơ đã lưu">
          {filtered.map((poem) => (
            <HistoryCard
              key={poem.id}
              poem={poem}
              onOpen={() => navigate(`/ket-qua/${poem.id}`)}
              onReuse={() => navigate('/sang-tac', { state: { openingVerse: poem.openingVerse, poetryForm: poem.poetryForm, authorStyle: poem.authorStyle, period: poem.period } })}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button variant="destructive" loading={deleting} onClick={handleDelete}>Xóa</Button>
          </>
        }
      >
        <p className="text-[#5f6673]">Bạn có chắc muốn xóa bài thơ này? Thao tác này không thể hoàn tác.</p>
      </Modal>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="secondary" onClick={() => setClearConfirm(false)}>Hủy</Button>
            <Button variant="destructive" onClick={handleClear}>Xóa tất cả</Button>
          </>
        }
      >
        <p className="text-[#5f6673]">Thao tác này sẽ xóa toàn bộ lịch sử trên trình duyệt và không thể hoàn tác.</p>
      </Modal>
    </div>
  )
}
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex-1 min-w-0">
          <h2 className="font-semibold text-[#252932] truncate text-base">{poem.title}</h2>
          <p className="text-sm text-[#5f6673] mt-0.5 italic font-serif line-clamp-1">"{poem.openingVerse}"</p>
        </div>
        {poem.saved && (
          <Badge variant="success" className="flex-shrink-0">Đã lưu</Badge>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        ...
        {poem.authorStyle && <Badge variant="outline">{poem.authorStyle}</Badge>}
      </div>
      ...
      <div className="flex items-center justify-between">
        <time className="text-xs text-[#7d8490]" dateTime={poem.createdAt}>
          {new Date(poem.createdAt).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' })}
        </time>
